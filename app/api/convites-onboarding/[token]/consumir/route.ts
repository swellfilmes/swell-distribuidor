...
import { consumirConviteBodySchema, lerBody } from '@/lib-web/validators';
...
export async function POST(
  ...
  const parsed = await lerBody(req, consumirConviteBodySchema);
  if (!parsed.ok) return parsed.resposta;
  const { nomeEmpresa, slug } = parsed.data;
